...
TRpredict <- function(calData, 
    ...
    preds <- cbind.data.frame(D47= mean(targety), 
                            D47error=sd(targety)/sqrt(length(targety)),
...
```{r}
reconstruct <- function(data, new_D47 , coeffi){
  OLS_recon <- TRpredict(data, new_D47 , coeffi[["OLS"]])
  print(OLS_recon)
  york_recon <- TRpredict(data, new_D47, coeffi[["york"]])
  print(york_recon)
  deming_recon <- TRpredict(data, new_D47 , coeffi[["deming"]])
  print(deming_recon)
  bayesian_recon <- TRpredict(data, new_D47 , coeffi[["bayesian"]])
  print(bayesian_recon)
  LSMC_recon <- TRpredict(data, new_D47 , coeffi[["LSMC"]])
  print(LSMC_recon)
  ODR_recon <- TRpredict(data, new_D47 , coeffi[["ODR"]])
  print(ODR_recon)
  QR_recon <- TRpredict(data, new_D47, coeffi[["QR"]])
  print(QR_recon)
  RLM_recon <- TRpredict(data, new_D47 , coeffi[["RLM"]])
  print(RLM_recon)
  TS_recon <- TRpredict(data, new_D47 , coeffi[["TS"]])
  print(TS_recon)
...
set.seed(3)
new_D47 <- rnorm(1000, 0.6, 0)
new_D47_error <- rnorm(1000, 0.6, 0.05)
recon_6 <- reconstruct(data_low, new_D47, coefficients_low)
recon_6_error <- reconstruct(data_low, new_D47_error, coefficients_low)
recon_6_error$uncertainty <- recon_6$temp - recon_6_error$temp
graph6_low <- drawGraph(recon_6_error,  
                    -max(abs(recon_6_error$uncertainty)),
                    max(abs(recon_6_error$uncertainty)))

set.seed(3)
recon_6 <- reconstruct(data_intermediate, new_D47, coefficients_intermediate)
recon_6_error <- reconstruct(data_intermediate, new_D47_error, coefficients_intermediate)
recon_6_error$uncertainty <- recon_6$temp - recon_6_error$temp
...
set.seed(3)
recon_6 <- reconstruct(data_high, new_D47, coefficients_high)
recon_6_error <- reconstruct(data_high, new_D47_error, coefficients_high)
recon_6_error$uncertainty <- recon_6$temp - recon_6_error$temp
...
set.seed(123)
recon_7 <- reconstruct(data_low, 0.7, 0, 1000, coefficients_low)
recon_7_error <- reconstruct(data_low, 0.7, 0.05, 1000, coefficients_low)
recon_7_error$uncertainty <- recon_7$temp - recon_7_error$temp
graph7_low <- drawGraph(recon_7_error,  
                    -max(abs(recon_7_error$uncertainty)),
                    max(abs(recon_7_error$uncertainty)))

set.seed(123)
recon_7 <- reconstruct(data_intermediate, 0.7, 0, 1000, coefficients_intermediate)
...
set.seed(123)
recon_7 <- reconstruct(data_high, 0.7, 0, 1000, coefficients_high)
...
set.seed(123)
recon_8 <- reconstruct(data_low, 0.8, 0, 1000, coefficients_low)
recon_8_error <- reconstruct(data_low, 0.8, 0.05, 1000, coefficients_low)
recon_8_error$uncertainty <- recon_8$temp - recon_8_error$temp
graph8_low <- drawGraph(recon_8_error,  
                    -max(abs(recon_8_error$uncertainty)),
                    max(abs(recon_8_error$uncertainty)))

set.seed(123)
recon_8 <- reconstruct(data_intermediate, 0.8, 0, 1000, coefficients_intermediate)
...
set.seed(123)
recon_8 <- reconstruct(data_high, 0.8, 0, 1000, coefficients_high)
