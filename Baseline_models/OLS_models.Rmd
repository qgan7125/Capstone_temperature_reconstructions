---
title: "OLS Model"
author: "Quan Gan"
date: "2/9/2022"
output: 
  html_document:
    toc: true
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE)

library(tidyverse)
library(dplyr)
library(ggplot2)
library(boot)
library(gridExtra)
library(knitr)
```

## load data in
There are three data sets with low, intermediate, and high errors.
```{r}
data_low <- read_csv("../Data/Dataset_S1.csv")
data_intermediate <- read_csv("../Data/Dataset_S2.csv")
data_high <- read_csv("../Data/Dataset_S3.csv")
```
## The functions for bootstrapping 1000 replicates with OLS models
```{r}
# https://www.statmethods.net/advstats/bootstrapping.html
# Boot library docs
# calculate the Intercept
OLS_Intercept <- function(formula, data, indices) {
  d <- data[indices,]                   # allows boot to select sample
  fit <- lm(formula, data=d)            # the OLS model
  return (coef(fit)[1])                 # get intercept
}

# calculate the slope
OLS_slope <- function(formula, data, indices) {
  d <- data[indices,]                   # allows boot to select sample
  fit <- lm(formula, data=d)            # the OLS model
  return (coef(fit)[2])                 # get slope
}

```

## Low error bootstrapping results

### Intercept for low error
```{r}

# set seed for production
set.seed(1234)

# bootstrapping with 1000 replications
results_Intercept <- boot(data=data_low, 
                statistic=OLS_Intercept,
                R=1000,
                formula=D47~Temperature)

plot(results_Intercept)

mean_Intercept_low <- mean(results_Intercept$t)
median_Intercept_low <- median(results_Intercept$t)
print(boot.ci(results_Intercept, type="bca"))
cat("Median Intercept:", median_Intercept_low)
cat("Mean Intercept:", mean_Intercept_low)


```

### Slope for low error
```{r}
# use the same seed to keep same results
set.seed(1234)

# bootstrapping with 1000 replications
results_Slope <- boot(data=data_low, 
                statistic=OLS_slope,
                R=1000,
                formula=D47~Temperature)

plot(results_Slope)

mean_slope_low <- mean(results_Slope$t)
median_Slope_low <- median(results_Slope$t)

print(boot.ci(results_Slope, type="bca"))
cat("Median Slope:", median_Slope_low)
cat("Mean Slope:", mean_slope_low)

```
### Comparing result to the true parameters
Plot confident interval area of the median slope/intercept of bootstrapping results
And comparing it to the true slope/intercept

I use the true data points to plot.
```{r}
data_low %>% 
  ggplot(aes(x = x_TRUE, y=y_TRUE)) + 
  geom_ribbon(aes(ymin = 0.0356 * x_TRUE + 0.2833, ymax = 0.0375 * x_TRUE + 0.2611), fill = "orange") +
  geom_point() +
  geom_abline(slope = 0.0369, intercept = 0.268, color = 'red') +
  geom_abline(slope = median_Slope_low, intercept = median_Intercept_low, color = 'yellow')
```
Mean slope/intercept VS Median slope/intercept
```{r}
data_low %>% 
  ggplot(aes(x = x_TRUE, y=y_TRUE)) +
  geom_point() +
  geom_abline(slope = 0.0369, intercept = 0.268, color = 'red') +
  geom_abline(slope = median_Slope_low, intercept = median_Intercept_low, color = 'yellow') +
  geom_abline(slope = mean_slope_low, intercept = mean_Intercept_low, color = 'purple')
```


## Intermediate error bootstrapping results

### Intercept for Intermediate error
```{r}

# set seed for production
set.seed(1234)

# bootstrapping with 1000 replications
results_Intercept <- boot(data=data_intermediate, 
                statistic=OLS_Intercept,
                R=1000,
                formula=D47~Temperature)

plot(results_Intercept)

mean_Intercept_Intermediate <- mean(results_Intercept$t)
median_Intercept_Intermediate <- median(results_Intercept$t)

print(boot.ci(results_Intercept, type="bca"))
cat("Median Intercept:", median_Intercept_Intermediate)
cat("Mean Intercept:", mean_Intercept_Intermediate)

```

### Slope for Intermediate error
```{r}
# use the same seed to keep same results
set.seed(1234)

# bootstrapping with 1000 replications
results_Slope <- boot(data=data_intermediate, 
                statistic=OLS_slope,
                R=1000,
                formula=D47~Temperature)

plot(results_Slope)

mean_Slope_Intermediate <- mean(results_Slope$t)
median_Slope_Intermediate <- median(results_Slope$t)

print(boot.ci(results_Slope, type="bca"))
cat("Median Slope:", median_Slope_Intermediate)
cat("Mean Slope:", mean_Slope_Intermediate)

```

### Comparing result to the true parameters
Plot confident interval area of the median slope/intercept of bootstrapping results
And comparing it to the true slope/intercept

I use the true data points to plot.
```{r}
data_intermediate %>% 
  ggplot(aes(x = x_TRUE, y=y_TRUE)) + 
  geom_ribbon(aes(ymin = 0.0332 * x_TRUE + 0.3131, ymax = 0.0367 * x_TRUE + 0.2709), fill = "orange") +
  geom_point() +
  geom_abline(slope = 0.0369, intercept = 0.268, color = 'red') +
  geom_abline(slope = median_Slope_Intermediate, intercept = median_Intercept_Intermediate, color = 'yellow')
```
Mean slope/intercept VS Median slope/intercept
```{r}
data_intermediate %>% 
  ggplot(aes(x = x_TRUE, y=y_TRUE)) +
  geom_point() +
  geom_abline(slope = 0.0369, intercept = 0.268, color = 'red') +
  geom_abline(slope = median_Slope_Intermediate, intercept = median_Intercept_Intermediate, color = 'yellow') +
  geom_abline(slope = mean_Slope_Intermediate, intercept = mean_Intercept_Intermediate, color = 'purple')
```


## High error bootstrapping results

### Intercept for high error
```{r}

# set seed for production
set.seed(1234)

# bootstrapping with 1000 replications
results_Intercept <- boot(data=data_high, 
                statistic=OLS_Intercept,
                R=1000,
                formula=D47~Temperature)

plot(results_Intercept)

mean_Intercept_high <- mean(results_Intercept$t)
median_Intercept_high <- median(results_Intercept$t)
print(boot.ci(results_Intercept, type="bca"))
cat("Median Intercept:", median_Intercept_high)
cat("Mean Intercept:", mean_Intercept_high)

```

### Slope for high error
```{r}
# use the same seed to keep same results
set.seed(1234)

# bootstrapping with 1000 replications
results_Slope <- boot(data=data_high, 
                statistic=OLS_slope,
                R=1000,
                formula=D47~Temperature)

plot(results_Slope)

mean_Slope_high <- mean(results_Slope$t)
median_Slope_high <- median(results_Slope$t)

print(boot.ci(results_Slope, type="bca"))
cat("Median Slope:", median_Slope_high)
cat("Mean Slope:", mean_Slope_high)
```


### Comparing result to the true parameters
Plot confident interval area of the median slope/intercept of bootstrapping results
And comparing it to the true slope/intercept

I use the true data points to plot.
```{r}
data_high %>% 
  ggplot(aes(x = x_TRUE, y=y_TRUE)) + 
  geom_ribbon(aes(ymin = 0.0283 * x_TRUE + 0.3721, ymax = 0.0327 * x_TRUE + 0.3186), fill = "orange") +
  geom_point() +
  geom_abline(slope = 0.0369, intercept = 0.268, color = 'red') +
  geom_abline(slope = median_Slope_high, intercept = median_Intercept_high, color = 'yellow')
```

Mean slope/intercept VS Median slope/intercept
```{r}
data_high %>% 
  ggplot(aes(x = x_TRUE, y=y_TRUE)) +
  geom_point() +
  geom_abline(slope = 0.0369, intercept = 0.268, color = 'red') +
  geom_abline(slope = median_Slope_high, intercept = median_Intercept_high, color = 'yellow') +
  geom_abline(slope = mean_Slope_high, intercept = mean_Intercept_high, color = 'purple')
```

## Median slopes/intercepts VS true parameters
```{r}
# integrate variables into the lists
DataNames <- c('Low', 'Intermediate', 'High', 'True')
Slopes <- c(median_Slope_low, median_Slope_Intermediate, median_Slope_high, 0.0369)
Intercepts <- c(median_Intercept_low, median_Intercept_Intermediate, median_Intercept_high, 0.268)

OLS_results <- do.call(rbind, Map(data.frame, name=DataNames, Slopes=Slopes, Intercepts=Intercepts))

Slopes_plot <- OLS_results %>% 
  ggplot(aes(x = name, y = Slopes,fill = name)) +
  geom_col() +
  geom_text(aes(label = round(Slopes, 4), y = Slopes + 0.005)) +
  labs(x = "Error Level",
       y = "Median Slope",
       fill = "Error Level")

Intercepts_plot <- OLS_results %>% 
  ggplot(aes(x = name, y = Intercepts ,fill = name)) +
  geom_col() +
  geom_text(aes(label = round(Intercepts, 4), y = Intercepts + 0.05)) +
  labs(x = "Error Level",
       y = "Median Intercept",
       fill = "Erro Level")

grid.arrange(Slopes_plot, Intercepts_plot, nrow = 2)
```
Mean slope/Intercept VS Median slope/Intercept
```{r}
meanIntercept <- c(mean_Intercept_low, mean_Intercept_Intermediate, mean_Intercept_high)
meanSlope <- c(mean_slope_low, mean_Slope_Intermediate, mean_Slope_high)
medianIntercept <- c(median_Intercept_low, median_Intercept_Intermediate, median_Intercept_high)
medianSlope <- c(median_Slope_low, median_Slope_Intermediate, median_Slope_high)

mat <- array(c(meanIntercept, medianIntercept, meanSlope, medianSlope), dim = c(3, 4))
colnames(mat) <- c("mean_Intercept", "median_Intercept", "mean_Slope", "median_Slope")
mat %>% 
  kable()
```
By the OLS model, we could know when there is a low error in the data. The OLS 
could produce the slope and intercept close to the true parameters but with the 
increment of error, the OLS model would be less precise and accurate to predict 
the relationship between D47 and temperature.