...
QR_boot <- function(formula, data, indices) {
  ...
  fit <- rq(formula, data=train)                      # the OLS model
...
QR <- function(seed, statistic, dataSet, formula, replicate){
  set.seed(seed)
...
```{r}
result_low <- QR(1234, QR_boot, data_low, D47~Temperature, 1000)
...
plot_low <- createPlot(data_low, 
           result_low, 
           "Quantile regression model with the low error data", 
           "Quantile regression model with the low error data. Red line as true coefficient. Yellow is Quantile regression fitted coefficient" )
...
result_intermediate <- QR(1234, QR_boot, data_intermediate, D47~Temperature, 1000)
...
plot_intermediate <- createPlot(data_intermediate, 
           result_intermediate, 
           "Quantile regression model with the intermediate error data", 
           "Quantile regression model with the intermediate error data. Red line as true coefficient. Yellow is Quantile regression fitted coefficient")
...
```{r}
result_high <- QR(1234, QR_boot, data_high, D47~Temperature, 1000)
...
plot_high <- createPlot(data_high, 
           result_high, 
           "Quantile regression model with the high error data", 
           "Quantile regression model with the high error data. Red line as true coefficient. Yellow is Quantile regression fitted coefficient")
